...
public class LessTask extends Task {
	...
	public void execute() throws BuildException {
		...
		if (this.compress == true) {
			try {
				// Load CSS file into RAM
				String cssData = readFileAsString(this.outFile);

				// Delete old file and open file writer
				new File(this.outFile).delete();
				FileWriter writer = new FileWriter(this.outFile);

				// Compress the CSS data using YUI compressor logic
				compress(cssData, writer, 1000);
    ...
    public void compress(String css, Writer out, int linebreakpos)
            throws IOException {
